                ...
                <div class="mx-list-grid">
                    @forelse($data as $item)
                        <a href="{{ url('detail/' . $param['model'] . '/' . $item['id']) }}" class="mx-card text-none-decoration">
                            <div class="mx-card__thumb mx-card__thumb--video">
                                <img src="{{ GetUrlByPath($item['cover'] ?? '') }}" alt="{{ $item['title'] ?? 'video' }}">
                                <span class="mx-video-badge"><i class="fa fa-play"></i></span>
                            </div>
                            <h3 class="mx-card__title">{{ $item['title'] ?? '未命名视频' }}</h3>
                    ...
                    @empty
                        <div class="mx-empty" style="grid-column:1 / -1;">当前还没有视频内容，请先在后台新增视频数据。</div>
                    @endforelse
                </div>
            </div>
