...
@php($queryCloudType = $currentFilters['cloud_type'] ?? ($queryParams['cloud_type'] ?? 'module'))
...
<div class="row page-header">
    <div class="col-lg-6 align-self-center ">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">{{getTranslateByKey("online_module_list")}}</li>
        </ol>
    </div>
</div>
                ...
                <div class="card-body">
                    <div class="row mb-3 pb-3">
                        ...
                        <div class="col-md-12">
                            ...
                            <a class="mr-3 @if(!$queryPlatform) text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query($queryParams))}}">{{getTranslateByKey('common_all')}}</a>
                            <a class="mr-3 @if($queryPlatform=="pc") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['platform' => 'pc'])))}}">PC</a>
                            <a class="mr-3 @if($queryPlatform=="moblic") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['platform' => 'moblic'])))}}">{{getTranslateByKey('mobile')}}</a>
                            <a class="mr-3 @if($queryPlatform=="H5") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['platform' => 'H5'])))}}">H5</a>

                        </div>
                        <div class="col-md-12">
                            {{getTranslateByKey('is_paid')}}：
                            <a class="mr-3 @if(!$queryIsfree) text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query($queryParams))}}">{{getTranslateByKey('common_all')}}</a>
                            <a class="mr-3 @if($queryIsfree=="n") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['isfree' => 'n'])))}}">{{getTranslateByKey('charged')}}</a>

                            <a class="mr-3 @if($queryIsfree=="y") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['isfree' => 'y'])))}}">{{getTranslateByKey('free')}}</a>

                        </div>
                        <div class="col-md-12">
                            {{getTranslateByKey('sort_attribute')}}：
                            <a class="mr-3 @if(!$querySort) text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query($queryParams))}}">{{getTranslateByKey('default_sort')}}</a>
                            <a class="mr-3 @if($querySort=="download") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['sort' => 'download'])))}}">{{getTranslateByKey('download_count')}}</a>
                            <a class="mr-3 @if($querySort=="commont") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['sort' => 'commont'])))}}">{{getTranslateByKey('score')}}</a>
                            <a class="mr-3 @if($querySort=="time") text-primary  @else text-muted @endif "
                               href="{{url("admin/cloud?".http_build_query(array_merge($queryParams, ['sort' => 'time'])))}}">{{getTranslateByKey('publish_time')}}</a>
                        </div>
                    </div>
                    <div class="row">
                                        ...
                                        <div class="">
                                                ...
                                                <p class="text-secondary small">
                                                    ...
                                                    <span class="float-right">
                                                        ...
                                                        @if($modules_install_data['type'] == "system")
                                                            {{getTranslateByKey('built_in_module')}}
                                                        @else
                                                            {{getTranslateByKey('feature_module')}}
                                                        @endif
                                                    </span>
                                                </p>
                                                <div class="mb-2">
                                            ...
                                            @if(!empty($local_data[$modules_install_data['identification']]))
                                                ...
                                                @else
                                                    ...
                                                    @if(session()->get('versionLimit')["module_".$modules_install_data["identification"]])
                                                        <div class="package-tip mt-2 text-danger">
                                                            {{getTranslateByKey('update_limit')}}：{{session()->get('versionLimit')["module_".$modules_install_data["identification"]]}}
                                                        </div>
                                                    @endif
                                                @endif
                                            @else
                                                @if($modules_install_data['install_allowed'])
                                                    <div class="d-inline-block cursor"
                                                         onclick="@if(!empty($modules_install_data['should_prompt_before_install'])) privatization('{{$modules_install_data['identification']}}','module') @else update('{{$modules_install_data['identification']}}','module') @endif">
                                                ...
                                                @if(!empty($modules_install_data['should_prompt_before_install']))
                                                    <div style="width: 600px;min-height:200px;max-height:350px;padding:10px 20px;scrollbar-width: thin;scrollbar-color: #888 #f1f1f1;display:none;"
                                                         class="h-privatization-top privatization_module_{{$modules_install_data['identification']}}">
                                                        ...
                                                        @if(!empty($modules_install_data['prompt_allows_continue_download']) || !empty($modules_install_data['prompt_requires_license_check']))
                                                            <div class="package-prompt-actions">
                                                                <button type="button" class="btn btn-primary btn-sm" onclick="layer.closeAll(); update('{{$modules_install_data['identification']}}','module')">{{$modules_install_data['prompt_continue_button_text'] ?? '继续下载'}}</button>
                                                            </div>
                                                        @endif
                                                    </div>
                                                @endif
