@if($f['formtype']=='selectMore')
    ...
    @if(!$f['showtype'] || $f['showtype']=='row')
        <div class="form-group row">
            ...
            <div class="col-lg-11">
                <select class="select-search" multiple {{$f['disabled']}}
                name="{{$f['identification']}}" id="{{$f['identification']}}"
                        @if($f['required']) required @endif>
                    @foreach($f['datas'] as $s)
                        <option value="{{$s['value']}}"
                                @if(in_array($s['value'],$f['value']?:[])) selected @endif
                        >{!! $s['name'] !!}</option>
                        @foreach($s['children']?:$s['child'] as $ss)
                            <option value="{{$ss['value']}}"
                                    @if(in_array($ss['value'],$f['value']?:[])) selected @endif
                            >　ㄴ {!! $ss['name'] !!}</option>
                        @endforeach
                    @endforeach
                </select>
